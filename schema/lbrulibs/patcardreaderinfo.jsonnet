...
local s = moo.oschema.schema("dunedaq.lbrulibs.patcardreaderinfo");
...
local info = {
      ...
      info : s.record("HALLinkInfo", [
	   ...
	   s.field("subscriber_connected"       , self.choice, 0, doc="mHAL device connected"                         ),
	   s.field("run_marker"                 , self.float8, 0, doc="Active run status"                             ),
	   s.field("sink_name"                  , self.string, 0, doc="Sink queue name, e.g. PAT"                     ),
	   ...
	   s.field("board_name"                 , self.string, 0, doc="Board name in XML"                             ),
	   s.field("dev_name"                   , self.string, 0, doc="Device name in the XML"                        ),
	   ], doc="mHAL link information"),
};
