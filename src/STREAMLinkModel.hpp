...
public:
  ...
  void init(const data_t& /*args*/) { TLOG_DEBUG(5) << "STREAMLinkModel init: nothing to do!"; }
...
private:
  ...
  std::chrono::time_point<std::chrono::system_clock> t_start =
    std::chrono::high_resolution_clock::now(); // Initial time when monitoring period starts
  ...
  virtual void get_info(opmonlib::InfoCollector& ci, int /*level*/)
  {
    ...
    std::chrono::time_point<std::chrono::system_clock> t_end =
      std::chrono::high_resolution_clock::now(); // End time when monitoring period ends
          ...
          try {
            ...
            if (is_toad) { // run TOAD receive loop
              ...
              for (int i = 0; i < num_ts; i++) {
                ...
                Payload->set_payload_size(nbytes);
                std::memcpy(static_cast<void*>(&Payload->data[0]), (void*)(&overlay), nbytes);
                delete[] buffer;
                // printf("payload size: %lld, %d, %d %d %d\n", overlay->tstmp, sizeof((Payload->data[0])),
                // (int)(Payload->data[0].get_size()), (Payload->get_payload_size()), sizeof(output[i]));
